...
| 11   | [Build & Push Application Image](#11-build--push-application-image-docker)      | Clone repo, build docker, push image  |
| 12   | [ECS Cluster + Task Definition](#12-ecs-cluster--task-definition)               | Container runtime                     |
| 13   | [Target Group + ALB](#13-target-group--alb)                                     | Load Balancer công cộng               |
| 14   | [ECS Service + Auto Scaling](#14-ecs-service--auto-scaling)                     | Chạy và scale containers              |
| 15   | [Verify & Monitor](#15-verify--monitor)                                         | Xác nhận mọi thứ hoạt động            |
| 16   | [Tính toán chi phí](#16-tính-toán-chi-phí)                                      | Breakdown chi phí hàng tháng          |
| 17   | [Cleanup](#17-cleanup)                                                          | Dọn dẹp khi không dùng                |
| 18   | [Checklist trước khi Go-Live](#18-checklist-trước-khi-go-live)                  | Luồng traffic đầy đủ                  |
...
![Architecture Overview](solution-architect/only-nutritrack-api-vpc.drawio.svg)
...
flowchart TB
    ...
    alb["nutritrack-api-vpc-alb-sg<br/>Inbound: HTTP 80 from 0.0.0.0/0<br/>Outbound: TCP 8000 to nutritrack-api-vpc-ecs-sg"]
    ecs["nutritrack-api-vpc-ecs-sg<br/>Inbound: TCP 8000 from nutritrack-api-vpc-alb-sg<br/>Outbound: HTTPS 443 and HTTP 80 to nutritrack-api-vpc-nat-sg"]
    s3_prefix["S3 Prefix List (com.amazonaws.ap-southeast-2.s3)<br/>S3 Cache qua VPCE"]
    nat["nutritrack-api-vpc-nat-sg<br/>Inbound: All from nutritrack-api-vpc-ecs-sg<br/>Inbound: SSH 22 from &lt;YOUR_PC_IP&gt;<br/>Outbound: All to 0.0.0.0/0"]
    your_ip["&lt;YOUR_PC_IP&gt;"]
    internet_out[Internet]

    internet_in -->|HTTP:80| alb
    alb -->|TCP:8000| ecs
    ecs -->|HTTPS 443 and HTTP 80| nat
...
> Policy này **KHÔNG có** `ecr:GetAuthorizationToken` vì dự án này dùng **Docker Hub** (`<your-dockerhub-username>/nutritrack-api-image`), không phải Amazon ECR.
...
## 11. Build & Push Application Image (Docker)

Trước khi định nghĩa ECS task, chúng ta cần có Docker image của NutriTrack được lưu trữ an toàn trên Docker Hub.

### 11.1 Clone Source Code
Clone repository chứa mã nguồn và Dockerfile của ứng dụng:
```bash
git clone https://github.com/justHman/NUTRI_TRACK
cd NUTRI_TRACK
```

### 11.2 Login, Build, và Push ARM64 Image
Vì AWS Fargate chạy trên nền tảng Graviton sử dụng kiến trúc ARM, bạn phải build image cho `linux/arm64`.

```bash
# 1. Đăng nhập vào Docker Hub
docker login

# 2. Bật multi-architecture builds (nếu chưa bật)
docker buildx create --use --name mybuilder

# 3. Build ARM64 image và push trực tiếp lên Docker Hub
# Thay 'imjusthman' bằng username Docker Hub của bạn nếu cần
docker buildx build ^
...
> **Tại sao lại dùng ARM64?** Vi xử lý AWS Graviton có tỷ lệ price/performance tốt hơn tới 20%. Kết hợp với Fargate Spot, điều này mang lại hiệu quả chi phí tối đa. Hãy đảm bảo task definition (ở bước tiếp theo) tham chiếu đúng image tag bạn vừa push ở đây.

---

## 12. ECS Cluster + Task Definition

### 12.1 Tạo ECS Cluster
...
> - `<your-dockerhub-username>/nutritrack-api-image:arm-latest` — Tag "latest" luôn cập nhật
> - `<your-dockerhub-username>/nutritrack-api-image:arm-090426` — Tag theo ngày (ddmmyy) để rollback
>
...
### 13.2 Tạo Application Load Balancer (ALB)
...
### 14.2 Cấu hình ECS Auto Scaling
...
#### 14.2.1 Bắt đầu từ Service vừa tạo ở bước 14.1
...
#### 14.2.2 Update Service để cấu hình Policy Step Scaling
...
#### 14.2.3 Tạo Policy Scale-Out (Khi CPU ≥ 70% → Thêm task)
...
#### 14.2.4 Tạo Policy Scale-In (Khi CPU ≤ 20% → Giảm task)
...
#### 14.2.5 Hoàn thành cập nhật Service cho Auto Scaling
...
#### 14.2.7 Verify Auto Scaling Policy + Alarms
...
### 15.1 Kiểm tra cơ bản — AWS Console
...
### 15.2 CloudWatch — Xem Logs Container
...
### 15.3 CloudWatch — Metrics & Alarms
...
3. Alarms Step Scaling đã cấu hình ở bước 14.2:
...
### 15.4 CloudTrail — Audit API Calls
...
### 15.5 Kiểm tra NAT Instance hoạt động
...
### 15.6 Test HA — Thử "kill" NAT Instance
...
### 15.7 CloudWatch Events — ECS Service Events
...
## 16. Tính toán chi phí

### 16.1 Thông số traffic sử dụng
...
### 16.4 So sánh: NAT Instance vs NAT Gateway
...
### 17.1 Tạm dừng (Không tốn tiền Fargate)
...
### 17.2 Xóa hoàn toàn (Cleanup full)
...
11. NAT Instances × 2 → Terminate → Delete
12. Security Groups × 3 → Delete
13. VPC → Delete
14. S3 Bucket → Empty → Delete
15. Secrets Manager → Delete (có 7-30 ngày grace period)
16. IAM Roles → Delete (nếu không dùng cho project khác)
```
...
- [ ] Image `<your-dockerhub-username>/nutritrack-api-image:arm-latest` accessible
